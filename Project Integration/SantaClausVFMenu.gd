...
func _ready():
	...
	if level_lost == 1 and 1 in ProgressData.santa_claus_vf_levels_unlocked:
		$Level1Button.visible = true
	else:
		$Level1Button.visible = false
	
	if level_lost == 2 and 2 in ProgressData.santa_claus_vf_levels_unlocked:
		$Level2Button.visible = true
	else:
		$Level2Button.visible = false
	
	if level_lost == 3 and 3 in ProgressData.santa_claus_vf_levels_unlocked:
